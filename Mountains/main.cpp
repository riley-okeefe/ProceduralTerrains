...
int main(int, char**) {
	...
	Window& window = app.create_window([&](Window&) {
		...
		// for reflection, we draw the whole scene in the FBO from a camera that 
		// is positioned below the current eye position and pointing upwards
		// essentially we get angle of incidence = angle of reflection
		// to get the reflection, the camera needs to move down and point 
		// upwards => move by current distance * 2 down and flip
		...
		// for refraction, draw the scene below the water height => blend this with 
	...
	window.set_title("Island/Mountains");
