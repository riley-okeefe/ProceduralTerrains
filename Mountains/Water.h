...
public:
    Water(float size_grid_x, float size_grid_y, float waterHeight) {
        ...
        // Create a quad mesh with the same size as terrain grid so that it covers the whole terrain. 
        // Quad mesh will have only two triangles and four vertices.
        // Get x and y values. The corners are (x, y) (-x, y), (x, -y), (-x, -y)
        float x = size_grid_x / 2;
        float y = size_grid_y / 2;
